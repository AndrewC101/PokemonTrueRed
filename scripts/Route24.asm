...
Route24Script3:
    ResetEvent EVENT_REMATCH  ; AndrewNote - reset rematch flag after beating Aizen
	ld a, [wIsInBattle]
	cp $ff
	...
	ld [wRoute24CurScript], a
	ld [wCurMapScript], a
	ResetEvent EVENT_NO_REMATCH_OPTION ; AndrewNote - enable rematch option after beating Aizen
	ret
...
Route24Text1:
	...
	CheckEvent EVENT_GOT_NUGGET
	jr nz, .got_item
; AndrewNote - refactored to allow rematches
.fightAizen
    SetEvent EVENT_BONUS_MONEY ; AndrewNote - give bonus money
	ld hl, Route24Text_51510
	call PrintText
	lb bc, PP_UP, 1 ; AndrewNote - Aizen gives pp up with each fight
	call GiveItem
	jr nc, .bag_full
	...
	jp TextScriptEnd
.got_item
    CheckEvent EVENT_NO_REMATCH_OPTION ; AndrewNote - don't offer rematch
    jr nz, .endTalk
