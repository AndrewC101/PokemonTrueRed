...
.noLinkBattle
    CheckEvent EVENT_ENEMY_MOVE_FIRST ; AndrewNote - used for enemy switches
    jp nz, .enemyMovesFirst
...
.invertOutcome
	...
	jr .playerMovesFirst
.enemyMovesFirst
    ResetEvent EVENT_ENEMY_MOVE_FIRST ; AndrewNote - used for enemy switches
	ld a, $1
	ldh [hWhoseTurn], a
	callfar TrainerAI
...
.notRival1Battle
    ; AndrewNote - battle events that need reset on a loss
    ResetEvent EVENT_NO_REMATCH_OPTION
...
EnemySendOutFirstMon:
	...
	jr .next3
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;     Decides which Pokemon to send out
; AndrewNote - Will now leave any SLP or FRZ mons to last
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
.next
	ld b, $ff
.next2
	inc b ; increment b to be the position on mon not currently out

	push de
    ld de, wEnemyPartyCount
    ld a, [de] ; a is party count
    pop de

    cp b ; are we on the last enemy mon
    jr nz, .continueSwitch ; continue if not

    ; if so have we been here before
    ; this would mean there are no non SLP FRZ mons left
    CheckEvent EVENT_SEND_OUT_FRZ_ENEMY
    jr nz, .continueSwitch ; continue if so

    ; if not then start again and allow frozen mon
    SetEvent EVENT_SEND_OUT_FRZ_ENEMY
    SetEvent EVENT_STOP_SWITCHING
    ld b, $FF
    jr .next2
...
.continueSwitch
	ld a, [wEnemyMonPartyPos]
	cp b
	jr z, .next2 ; try again if this is the active mon
	...
	ld a, [hl]
	or c
	jr z, .next2 ; if hp = 0 try again

    ; is mon is sleep or frz if not then send out
	inc hl
	inc hl
	ld a, [hl] ; status
	and (1 << FRZ) | SLP
    ...
    ; if so check if we can send it out
    CheckEvent EVENT_SEND_OUT_FRZ_ENEMY
    jr nz, .next3 ; if so just continue with frozen enemy

    ; if not go back
    jr .next2
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
.next3
    ResetEvent EVENT_SEND_OUT_FRZ_ENEMY
	ld a, [wWhichPokemon]
...
TryRunningFromBattle:
    ...
    ; AndrewNote - Mewtwo unique run text
	ld a, [wCurOpponent]
    cp MEWTWO
    jp z, .mewtwoBlock    ; can't escape mewtwo
...
.atLeastTwoMovesAvailable
	ld a, [wIsInBattle]
	dec a
	jr nz, .trainerAI ; trainer AI if this isn't a wild battle
	;jr .chooseRandomMove

    ; AndrewNote - give wild Mewtwo AI
	ld a, [wCurOpponent]
    cp MEWTWO
    jr nz, .chooseRandomMove ; wild encounter not mewtwo
.trainerAI
...
.MonHurtItselfOrFullyParalysed
	...
	; AndrewNote - adding INVULNERABLE to fix immortal glitch with FLY DIG
	and ~((1 << STORING_ENERGY) | (1 << THRASHING_ABOUT) | (1 << CHARGING_UP) | (1 << INVULNERABLE) | (1 << USING_TRAPPING_MOVE))
	ld [hl], a
	ld a, [wPlayerMoveEffect]
	cp FLY_EFFECT
...
.calcCriticalHitProbability
	...
	jr nz, .focusEnergyUsed      ; bug: using focus energy causes a shift to the right instead of left,
	                             ; resulting in 1/4 the usual crit chance - FIXED
	; AndrewNote - half crit rate by removing shift
	;sla b                        ; (effective (base speed/2)*2)
	; commented out above so right now b = baseSpeed/2 rather than baseSpeed
	jr nc, .noFocusEnergyUsed
	ld b, $ff                    ; cap at 255/256
	jr .noFocusEnergyUsed
.focusEnergyUsed
    ; AndrewNote fix focusEnergy by shifting left instead of right
	sla b                        ; effective base speed
.noFocusEnergyUsed
...
.Loop
	...
	srl b                        ; /2 for regular move (effective (base speed / 4)) - half of normal
...
.monHurtItselfOrFullyParalysed
	...
	; AndrewNote - add INVULNERABLE to remove immortal glitch on FLY DIG for enemies
	and ~((1 << STORING_ENERGY) | (1 << THRASHING_ABOUT) | (1 << CHARGING_UP) | (1 << INVULNERABLE) | (1 << USING_TRAPPING_MOVE))
	ld [hl], a
	ld a, [wEnemyMoveEffect]
	cp FLY_EFFECT
...
; AndrewNote - max DVs for pokemon after beating elite 4
    CheckEvent EVENT_BEAT_ELITE_4 ; max DVs after E4
    jr z, .calcDVs

    call Random ; generate random IVs
	or $FF ; pokemon given to player have max DVs
	ld b, a
	call Random
	or $FF ; pokemon given to player have max DVs
	jr .storeDVs

; AndrewNote - good dvs for one time Pokemon - 10 DVs at worst
.calcDVs
    CheckEvent EVENT_GOOD_DVS
    jr z, .randomDVs

    call Random ; generate random IVs
	or $AA ; pokemon have 10 DVs at worst
	ld b, a
	call Random
	or $AA ; pokemon have 10 DVs at worst
	ResetEvent EVENT_GOOD_DVS
	jr .storeDVs
...
; enemies with stat exp would have missing hp otherwise

;make sure it's current hp = it's max hp
	ld a, [wIsInBattle]
	cp $2 ; is it a trainer battle?
	jr nz, .nottrainer3

; has poke been sent out before, if so ignore hp
    push bc
	push hl
	call CheckAISentOut
	pop hl
...
;set hp equal to max hp for first time set out
	ld a, [wEnemyMonMaxHP]
	ld [wEnemyMonHP], a
	ld a, [wEnemyMonMaxHP+1]
	ld [wEnemyMonHP + 1], a
...
; AndrewNote - mark mon as sent out, so we can not set its hp to max when switched out
	push af
	ld a, [wIsInBattle]
	cp $2 ; is it a trainer battle?
...
; AndrewNote - function to clear flags set above
; this is needed or the flags would persist between battles
ClearAISentOut:
	ld a, [wFontLoaded]
	res 6, a
	ld [wFontLoaded], a
...
; AndrewNote calcs stat exp
; each stat can have up to 65535 (2^16 - 1)
;requires a, b, de, and wCurEnemyLVL
CalcEnemyStatEXP:
; if max stat exp event is set
; set 655 stat exp per level
; this gives max 64/64 at lvl 100
    CheckEvent EVENT_MAX_STAT_EXP
    jr z, .mediumExp
	ld a, $02
	ld [hMultiplicand], a
	ld a, $8F
...
; AndrewNote - Code that dealt with BadgeBoosts has been deleted
; BadgeBoost is no longer a thing, it was dumb
